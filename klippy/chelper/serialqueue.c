...
// Restart the transport workers after an EOF while retaining all protocol
// sequence numbers and command queues.  The caller has already replaced the
// underlying descriptor (normally with dup2()), so the integer stored in the
// poll set remains valid.  Return one when restarted, zero when it was still
// running, and minus one on thread creation failure.
int __visible
serialqueue_reconnect(struct serialqueue *sq)
{
    ...
    // A partial frame from the disconnected device must not be prefixed to
    // the first frame from the re-enumerated endpoint.
    sq->input_pos = 0;
    sq->need_sync = 0;

    // Retain only frames that were actually put on the wire.  Those frames
    // are in sent_queue and must remain available to ARQ so the MCU and host
    // keep the same sequence number.  Commands still in ready/upcoming were
    // never transmitted.  They may have been generated by timers or macros
    // during the outage and replaying them as a reconnect burst is unsafe.
    // Convert unsent notifications into local cancellation records so their
    // Python waiters do not remain blocked after the purge.
    struct list_head canceled;
    list_init(&canceled);
    pthread_mutex_lock(&sq->lock);
    while (!list_empty(&sq->ready_queues)) {
        ...
        list_del(&cq->ready.node);
        while (!list_empty(&cq->ready.msg_queue)) {
            struct queue_message *qm = list_first_entry(
                &cq->ready.msg_queue, struct queue_message, node);
            list_del(&qm->node);
            if (qm->notify_id) {
                qm->len = 0;
                qm->sent_time = qm->receive_time = 0.;
                list_add_tail(&qm->node, &canceled);
            } else {
                message_free(qm);
            }
        }
    ...
    sq->transmit_requests.upcoming_bytes = 0;
    sq->transmit_requests.need_kick_clock = MAX_CLOCK;
    sq->transmit_requests.min_release_clock = MAX_CLOCK;
    pthread_mutex_unlock(&sq->transmit_requests.lock);
    if (list_empty(&sq->sent_queue))
        ...
        pollreactor_update_timer(sq->pr, SQPT_RETRANSMIT, PR_NOW);
    pollreactor_update_timer(sq->pr, SQPT_COMMAND, PR_NEVER);
    pthread_mutex_unlock(&sq->lock);
    if (!list_empty(&canceled))
        receive_append_wake(&sq->receiver, &canceled);

    pollreactor_reset(sq->pr);
    ret = pthread_create(&sq->tid, NULL, background_thread, sq);
    if (ret) {
        report_errno("pthread_create", ret);
        pollreactor_do_exit(sq->pr);
        return -1;
    }
    return 1;
