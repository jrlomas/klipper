...
class WindowComparator:
    def __init__(self, config):
        ...
        self.lower_threshold = int(config.getfloat('lower_threshold', 0.25, minval=0, maxval=1)*4095)
        self.upper_threshold = int(config.getfloat('upper_threshold', 0.75, minval=0, maxval=1)*4095)
        ...
        if self.upper_threshold <= self.lower_threshold:
            raise config.error("upper_threshold must be greater than lower_threshold")
        ...
        self.mcu.register_response(self._handle_comp_upper_trigger, "comp_upper_trigger")
        self.mcu.register_response(self._handle_comp_lower_trigger, "comp_lower_trigger") 
        ...
        logging.info("Window comparator '%s' configured: pin=%s, upper=%d, lower=%d",
                    self.name, self.pin, self.upper_threshold, self.lower_threshold)
    
    cmd_COMP_QUERY_STATE_help = "Query comparator state: COMP_QUERY_STATE COMP=<name>"
    def cmd_COMP_QUERY_STATE(self, gcmd):
        ...
        response = "Comparator '%s': pin=%s, thresholds=%d/%d" % (
            comp_name, status['pin'], status['lower_threshold'], status['upper_threshold'])
    ...
    def _build_config(self):
        ...
        self.mcu.register_response(self._handle_comp_state, "comp_state", self.oid)
    ...
    def _handle_comp_upper_trigger(self, params):
        ...
        logging.info("Window comparator '%s' upper threshold exceeded on pin %s", 
                    self.name, pin)
    ...
    def _handle_comp_lower_trigger(self, params):
        ...
        logging.info("Window comparator '%s' lower threshold exceeded on pin %s", 
                    self.name, pin)
    ...
    def _handle_comp_state(self, params):
        ...
        logging.debug("Comp state: pin=%s, in_window=%s, upper_out=%s, lower_out=%s",
    ...
    def set_irq_enable(self, upper_enable=True, lower_enable=True):
        ...
        raise self.printer.command_error("IRQ management removed - fixed edge triggering enabled")
