    ...
    def setUpClass(cls):
        if not has_cv2_build():
            if os.environ.get('HELIX_REQUIRE_LIVE'):
                raise RuntimeError(
                    "CI expected a linuxprocess WANT_CONSOLE_FRAMING_V2 build")
            raise unittest.SkipTest(
                "needs a linuxprocess build with WANT_CONSOLE_FRAMING_V2")
        if os.path.lexists(PTY):
            os.unlink(PTY)
        ...
        while time.time() < deadline:
            if os.path.islink(PTY) and os.path.exists(PTY):
                break
            if cls.proc.poll() is not None:
                raise RuntimeError("linuxprocess exited before publishing PTY")
            time.sleep(0.05)
        else:
            raise RuntimeError("linuxprocess did not publish a usable PTY")
