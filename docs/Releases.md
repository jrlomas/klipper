...
> Board runtime bring-up is now qualified on the supervised USB V0 Pico/EBB36
> rig, including interrupt homing, normal-G1 quintic motion, and deterministic
> pulse replay. The remaining board matrix, CAN repetition, scope-level timing,
> and downstream OAMS product-key provisioning remain; the latter stays
> deliberately fail-closed. The remaining motion-critical paths have not yet
> been validated across the intended fleet. Treat 0.9 as a developer preview;
> every
...
> correct what that exposes. That validated
