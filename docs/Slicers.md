...
default is frequently "Marlin" and that works well with Helix. The
"Smoothieware" setting also works well with Helix.
...
the Helix config file instead - such as: `[gcode_macro START_PRINT]`
...
Helix configuration may make it easier to tweak the printer's start
...
## Large retraction settings may require tuning Helix

The maximum speed and acceleration of retraction moves are controlled
in Helix by the `max_extrude_only_velocity` and
...
If using a large retraction, consider tuning Helix's
...
trigger Helix's maximum extrusion cross-section check. Consider
using Helix's [pressure advance](Pressure_Advance.md) or the regular
...
zero. Consider using Helix's [pressure advance](Pressure_Advance.md)
instead.

## Disable any "advanced extruder pressure" settings

Some slicers advertise an "advanced extruder pressure" capability. It
is recommended to keep these options disabled when using Helix as
they are likely to result in poor quality prints. Consider using
Helix's [pressure advance](Pressure_Advance.md) instead.
...
desirable extruder pressure. Helix, however, utilizes precise
kinematic calculations and timing. When Helix is commanded to make
...
Helix's maximum extrusion cross-section check.
