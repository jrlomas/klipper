...
for basic analysis.  However, Helix's `scripts` folder contains the
...
Like most graphing tools provided by Helix, `graph_mesh.py` requires
the `matplotlib` and `numpy` python dependencies. In addition, connecting
to Helix via Moonraker's websocket requires the `websockets` python
...
- `input`: A required positional argument containing a path or url
  ...
  - A path to Helix's Unix Domain Socket
...
- `-s`:  Optionally scales a plot using the `axis_minimum` and `axis_maximum`
  values reported by Helix's `toolhead` object when the dump file was
  generated.
...
For example, to plot an animated rapid path, connecting via Helix's unix
socket:
...
As with the `plot` command, the `<input>` must be a path to Helix's
...
The `<input>` should be a path to Helix's unix socket or
