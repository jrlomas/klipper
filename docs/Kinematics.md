...
working on the Helix software as well as users interested in better
understanding the mechanics of their machines.

## Acceleration

Helix implements a constant acceleration scheme whenever the print
head changes velocity - the velocity is gradually changed to the new
speed instead of suddenly jerking to it. Helix always enforces
...
do not skip or put excessive stress on the machine. Helix limits the
...
Helix implements constant acceleration. The key formula for constant
acceleration is:
```
...
Helix uses a traditional "trapezoid generator" to model the motion
...
However, in Helix, junction speeds are configured by specifying the
...
Helix also implements a mechanism for smoothing out the motions of
...
and increases the noise. Helix implements a mechanism to ensure
...
Helix implements this feature by tracking both a regular move
...
Helix code. Outside of these kinematic classes, everything is
...
these formulas, Helix can determine the ideal time that the stepper
...
However, to avoid extreme cases, Helix enforces a maximum ceiling on
...
Helix implements extruder motion in its own kinematic class. Since
...
make sudden velocity changes. Helix implements "smoothing" of the
