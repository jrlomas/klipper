...
| STM32G0B1 automatic consumers | EBB36 PA3 hotend plus internal temperature and the USB CAN-FD bridge internal temperature run through their respective DMA engines in forced mode. Both use 16x hardware oversampling with native-scale shift followed by the normal 8-sample software average. Klipper reached Ready with distinct live readings and no fallback or ADC fault. |
