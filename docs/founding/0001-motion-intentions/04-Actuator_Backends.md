...
* **Step generation:** a quotient/remainder crossing recurrence for
  constant-velocity cruise segments and the incremental Newton step-time
  solver specified in
  ...
  `step_pulse_ticks` / inverted-pin handling. Cruise pays its divisions once
  when the segment loads; recurring pulse IRQs advance using additions and a
  remainder carry only. Keeping division out of the recurring timer-interrupt
  path is required on division-poor MCUs so other precision timer clients
  (notably software TMC UART) retain their sampling deadlines.
