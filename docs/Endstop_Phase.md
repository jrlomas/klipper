...
carriage should always be at the same stepper motor phase. Helix's
...
mode (ie, not stand-alone mode) then Helix can query the stepper
...
use that phase. (So, in future homing operations, Helix will obtain
...
* It is possible to use this system with traditional (non-Trinamic)
  ...
  is reset. (If the two are always reset together then Helix can
