...
| `RESUME_MOTION` | Establish a fresh all-MCU execution epoch/grant, reconcile every joint from its execution log, and resume the print; blocks if the grant cannot be committed or a joint's homing was genuinely lost. During a trajectory recovery hold, ordinary `RESUME` and the Mainsail resume action automatically route here. |
...
| `HELIX_CAN_RX_STRESS MCU=<name> [ITERATIONS=100] [HOLD_US=2000]` | No-motion/no-heater field regression for an FDCAN node. Queues three near-maximum command records while applying a firmware-capped IRQ hold; inspect `HELIX_CAN_STATUS` or `canbus_stats` afterward for per-FIFO loss/high-water. This complements, but does not replace, the host short-record carrier-credit regression. Requires built-in MCU self tests. |
